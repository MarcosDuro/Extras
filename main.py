...
- - - - - - - - - - - - - - - - - - - -- - - - - - - - - - - 
𝗖𝗼𝘂𝗻𝘁𝗿𝘆 ➟ <code>{country_name}</code>
    ═══════════
[𝗨𝗦𝗔 𝗔𝗗𝗗𝗥𝗘𝗦𝗦 🇺🇸]
•  𝗦𝗧𝗥𝗘𝗘𝗧: <code>{loca} {nm}</code>
•  𝗖𝗜𝗧𝗬: <code>{city}</code>
•  𝗦𝗧𝗔𝗧𝗘: <code>{state}</code>
•  𝗭𝗜𝗣: <code> {postcode}</code>
...
"""    
    ...
    api = requests.get(f"https://randomuser.me/api/?nat=us&inc=name,location").json()
